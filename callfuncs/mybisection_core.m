function diagnostic = mybisection_core(Constraints,Objective,options,user_lower)
...
if isempty(user_lower)
    lower = 0; % Typically a good guess
else
    lower=-user_lower;  %for negative signed decay-rate
end
...
if isinf(upper)
    ...
    while ~flag
        ...
        upper = upper + 0.1*lower; i = i+1; % upper = upper + 2^i;i = i+1;
        try                         
            solvertime = tic;
            if options.verbose;        
                fprintf([' (ok), ' num2str(upper*the_sign)]);

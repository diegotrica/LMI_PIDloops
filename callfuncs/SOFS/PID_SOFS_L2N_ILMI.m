...
%	bound on Hinf norm by L2 function norms. SDP seeks to maximize 
%	system decay rate solving the generalized eigenvalue problem (GEVP) 
%	using the gevp command.
%
...
% Psi_ = X_'*(Bu_*Bu_')*P_ + P_*(Bu_*Bu_')*X_ - X_'*(Bu_*Bu_')*X_
    ...
    lmiterm([neq 1 1 0],(Cy_{i}(1:Ny,:)'*Cy_{i}(1:Ny,:)));
    lmiterm([neq 2 1 P_],Bw_{i}',1);
	lmiterm([neq 3 1 P_],Bu_{i}',1);
	lmiterm([neq 3 1 K_],1,-Cy_{i});

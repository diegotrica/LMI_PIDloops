function [KP,KI,KD,TI,TD,alp,P_,Sgm_FP,epsl_FP,Sgm_Pprp,epsl_Pprp]=...
	PID_SOFS_YALMIP(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,law)
...
%	interface. SDP seeks to maximize system decay rate solving 
%	the generalized eigenvalue problem (GEVP) using YALMIP bisection
%	built-in function.
%
...
switch law
    case 'P'
[vars,lmis,obj]=LMI_P(Poly,KPlb,KPub);
    
	case 'PI'
[vars,lmis,obj]=LMI_PI(Poly,KPlb,KPub,TIlb,TIub);

    case 'PID'
[vars,lmis,obj]=LMI_PID(Poly,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub);
end 
...
function [vars,lmis,obj]=LMI_P(Poly,KPlb,KPub)
%Parameters ---------------------------------------------------------
...
%F_(KP)
    ...
    lmis=[lmis,...
	    ...
	    F_{i} <= 2*(alp + 1e-6)*P_{i}... %(alp + 1e-6) to de-homogenize LMI
	    ];
end
...
function [vars,lmis,obj]=LMI_PI(Poly,KPlb,KPub,TIlb,TIub)
%Parameters ---------------------------------------------------------
Nx=Poly.Nx; 	%State variables size
Ny=Poly.Ny;		%Output variables size
Nl=Poly.Nl;		%Number of polytope LTI system verteces
...
function [vars,lmis,obj]=LMI_PID(Poly,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub)
%Parameters ---------------------------------------------------------
Nx=Poly.Nx; 	%State variables size
Ny=Poly.Ny;		%Output variables size
Nl=Poly.Nl;		%Number of polytope LTI system verteces

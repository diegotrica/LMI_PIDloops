function [KP,KI,KD,TI,TD,alp,P_,iter]=...
	PID_SOFS_YALMIP_ILMI(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,law)
%	Semidefinite programming (SDP) problem to obtain feedback PID 
%	controller gain which maximizes system decay rate using YALMIP
%	interface. SDP seeks to maximize system decay rate solving the 
%	generalized eigenvalue problem (GEVP) using YALMIP bisection 
%	built-in function.
%
...
%     'sedumi.stepdif',2,...'sedumi.stepdif',2,...'sedumi.stepdif',0,...
%     'sedumi.free',1,...'sedumi.free',0,...
%     'sedumi.sdp',1,...'sedumi.sdp',1,...
%     'verbose',1);
optOP1=sdpsettings(...
    'solver','bisection',...
...
while iter<maxiter
	...
	[vars,lmis,obj]=LMIOP1(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,X_,law);   		%Step 2
...
% iteration step, reducing algorithm computational effort
	...
	[vars,lmis,obj]=LMIOP2(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,X_,alp,law);	%Step 4
...
function [vars,lmis,obj]=...
	LMIOP1(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,X_,law)
...
% F{i} < 0
% F{i}(P_,K_)
	% = A_'*P_ + P_*A_ + P_*((Bw_/gL2^2)*Bw_')*P_ - P_*Bu_*K_*Cy_ - Cy_'*K_'*Bu_'*P_ < 2*alp*P_
	% = A_'*P_ + P_*A_ + P_*((Bw_/gL2^2)*Bw_')*P_ - P_*(Bu_*Bu_')*P_ + (Bu_'*P_-K_*Cy_)'*(Bu_'*P_-K_*Cy_) - (K_*Cy_)'*(K_*Cy_) < 2*alp*P_
% dropping - (K_*Cy_)'*(K_*Cy_) yields to:
	% = A_'*P_ + P_*A_ - Psi_ + P_*((Bw_/gL2^2)*Bw_')*P_ + (Bu_'*P_-K_*Cy_)'*(Bu_'*P_-K_*Cy_) < 2*alp*P_
...
lmis=[lmis,...
    F_{i} <= eps*eye(Nx_+Ny);...
    F11-Sgm_ <= 2*(alp+1e-6)*P_; %(alp + 1e-6) to de-homogenize LMI
	];
end
...
function [vars,lmis,obj]=...
	LMIOP2(Poly,Tk,PhiD,KPlb,KPub,TIlb,TIub,TDlb,TDub,X_,alp,law)
...
% F{i}(P_,K_)
	...
	% = A_'*P_ + P_*A_ + P_*((Bw_/gL2^2)*Bw_')*P_ - P_*(Bu_*Bu_')*P_ + (Bu_'*P_-K_*Cy_)'*(Bu_'*P_-K_*Cy_) - (K_*Cy_)'*(K_*Cy_) < 2*alp*P_
% dropping - (K_*Cy_)'*(K_*Cy_) yields to:
	% = A_'*P_ + P_*A_ - Psi_ -2*alp*P_ + P_*((Bw_/gL2^2)*Bw_')*P_ + (Bu_'*P_-K_*Cy_)'*(Bu_'*P_-K_*Cy_) < 0
% By schur complements
